...
**Portfolio** — `Portfolio (Id, UserId, Name, Description?, Currency, IsArchived)`; `Asset (Id, UserId, PortfolioId, AssetClass, ValuationMode, Name, Currency, Quantity, ManualValueAmount?, ManualValueDate?, InstrumentId?, Version, xmin)`; `Transaction (Id, UserId, AssetId, Type, Quantity, UnitPriceAmount, FeeAmount, Date, xmin)`. Delete guards query the children (`Assets.AnyAsync`, `Transactions.AnyAsync`) — no denormalized counters. `Asset.Version` is the per-asset ordering counter on `AssetPositionChanged`, written only by `PositionEventPublisher`; `xmin` stays the concurrency token and is a separate thing (it doesn't move when only the portfolio's archived flag changes).
