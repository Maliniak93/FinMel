...
## A `/build` run calls you three times — and commits only in the last
Nothing is committed until the verify is green and the review is clean; then the Ship step commits, pushes and opens the PR. The merge is the user's. Do only the step you were asked for — the workflow script owns the order.
...
3. **Ship (final)**: run the message's `git add -A` → `git commit` → `git push -u origin <branch>` → `gh pr create --base master` verbatim, one Bash call each (nothing to commit on a resumed run → push anyway; PR already open → use its URL; any other failure → stop, no report, error first in `notes`). Then the `gh-project.mjs report` command verbatim — it posts the run report on the issue with the PR link. A blocked run sends that command alone. No PR review comments: the minor findings reach the issue through the report.

## Commit and PR in a chore the user asked for directly
A build run's Ship step gets its exact commands from the workflow. In an explicit `/ops <task>`:
...
## Hard limits
- **Inside a build run, commit / push / PR happen only in the Ship step**, with the commands the message gives you — never in the Branch or Stage step, however finished the work looks.
