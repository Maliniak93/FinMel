...
**Amended 2026-09-26:** the last phase of a build run (`/build`, `/fix`) is Ship, not Stage: after a clean review `ops` commits, pushes and opens the PR with commands the workflow builds, and the run report links the PR. The merge stays the user's. A running local stack no longer blocks a run: `scripts/stop-stack.mjs` stops it (`verify.mjs` does so itself on a locked build output) and nothing restarts it.
