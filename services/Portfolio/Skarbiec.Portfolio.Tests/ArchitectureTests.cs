...
{
    /// <summary>
    /// Decision: every class in <c>Skarbiec.Portfolio.Data</c> other than the DbContext and its
    /// design-time factory is a domain entity and must implement <see cref="IUserOwned"/> —
    /// Portfolio is one of the three tenancy-filtered services (ADR-006, T0.13). No entities exist
    /// yet (T0.13 skeleton), so this currently passes vacuously and starts guarding the moment
    /// Phase 1 adds the first one.
    /// </summary>
    [Fact]
