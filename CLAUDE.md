...
Agents never run git — only `ops`: in a build run `gh issue develop` (cuts the issue's linked branch), `git add -A`, and in the final Ship step commit, push and `gh pr create`; otherwise only in an explicit `/ops` chore. No agent merges.
