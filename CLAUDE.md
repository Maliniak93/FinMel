...
Agents never run git — only `ops`, and inside a build run only `gh issue develop` (cuts the issue's linked branch) and `git add -A`; commit/push/PR happen in an explicit `/ops` chore or by your own hand.
